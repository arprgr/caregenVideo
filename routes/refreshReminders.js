...
router.post('/', function(req, res) {
    ...
    var clientDateTime = req.body.clientDateTime;
    var receiverEmailid = req.body.receiverEmailid;

    console.log('Refreshing reminders!! for ' + receiverEmailid + 'at ' + clientDateTime);
    
    models.Reminders.findAll({ attributes: ['id', 'senderEmailId', 'receiverEmailId', 'messageType', 'status', 'deliverAt', 'timeZone', 'Repeat', 'Expired' , 'lastDeliveredAt'],
    where: {
          receiverEmailId: receiverEmailid , $not: [ { Expired : 'Yes' } ]}
        ...
        if (!Reminders) {
         ...
         console.log('** from refresh reminder: clientDateTime = ' + clientDateTime + "reminders = "  + Reminders);      
         Reminders.forEach(function (reminder) {
                        var deliverAt = reminder.deliverAt;
                        var timeZone = reminder.timeZone;
                        var repeat = reminder.Repeat;
                        var id = reminder.id;
                        var senderEmailId = reminder.senderEmailId ;
                        var receiverEmailId = reminder.receiverEmailId;
                        var lastDeliveredAt = reminder.lastDeliveredAt ;
                        var expired = reminder.Expired;

                        var date = new Date(deliverAt);
                        var rightNow = new Date(clientDateTime);
                        var lastDeliveredAtDate; var actualDeliveryDate;

                        if (lastDeliveredAt != 'Never') { lastDeliveredAtDate = new Date(lastDeliveredAt) };

console.log("** from refresh reminder: clientDateTime = " + rightNow + "reminder date=" + date + "Reminder id " + id + 'lastDeliveredAt:' + lastDeliveredAt);
        ...
        console.log('*_*_* getting in to updates repeat=' + repeat + 'lastDeliveredAt=' + lastDeliveredAt);      
                        ...
                        if (repeat == 'Yes') {
                            if (lastDeliveredAt != 'Never') {  
                    console.log('This is a repeat reminder thats been sent before' + 'deliverhours:' + deliverAtHours + 'deliverAtMinute:' + deliverAtMinute);
                         if (deliverAtHours == nowHours && deliverAtMinute == nowMinutes && nowDate > lastDeliveredAtDay) { 
    ...
    console.log('This is a repeat reminder thats never been sent before' + 'deliverhours:' + deliverAtHours + 'deliverAtMinute:' + deliverAtMinute + 'nowHours' + nowHours + 'nowMinutes' + nowMinutes);
                if (deliverAtHours == nowHours && deliverAtMinute == nowMinutes) { 
                              models.Reminders.update (
                                { 
                                    Repeat : 'Yes',
                                    lastDeliveredAt : actualDeliveryDate 
