...
reader.onloadend = function() {
 ...
 function writeToCloudinary(fileToWrite) {
    ...
    var selectedNames='';
     ...
     var recepientList = document.querySelector('input#recepients');
     var senderEmailid = document.querySelector('input#sender');
    
     jsonString = JSON.parse(recepientList.value);
     
     for (i in jsonString) { 
        var innerVal =  jsonString[i];
        for (s in innerVal) { 
           if(selectedNames != '')    
            selectedNames = selectedNames + ',' + s;
           else
            selectedNames = s;
        }    
     }
